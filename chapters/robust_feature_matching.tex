...
The next class in this hierarchy that is inherited from FeatureBase is Feature class. The main class that consists of a  \detokenize{boost::shared_ptr} of FeatureBase that represent a position of a feature point and a \detokenize{Math::Pose} object that uses for storing the position of object respect to the camera. The \detokenize{Math::Pose} involves a rotation matrix a vector for translation. To support the OpenCV feature point (implemented by OpenCV), class OpenCVFeature extracted from Feature class. It consists of a \detokenize{cv::KeyPoint} for string the data of feature point and \detokenize{cv::Mat} for their descriptor vector (binary or float). The cvAKAZEFeature is a sub class of OpenCVFeature that uses for saving the value of AKAZE's key point and descriptor. The \autoref{fig:cv_akaze} illustrates the hierarchy of cvAKAZEFeature.
...
\subsection {RobustFeatureMatching}
...
% \centering
  % \begin{tabular}{| c || c | c | c | c | c |}
      % \hline
      % Data Set & Brute Force & Nearest Neighbor & Symmetric & Epipolar Constraint \\ \hline \hline
      % Mean & 555.239667 & 529.759167 & 532.873833 & 563.157 \\  \hline
      % Standard Deviation & 570.340981 & 557.464191 & 558.289539 & 559.798208 \\  \hline
  % \end{tabular}
  % \caption{The mean and standard deviation of time} \label{tab:time_matching_statistic}
% \end{table}

% In the case of computation time, 
